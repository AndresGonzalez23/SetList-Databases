Public Class AlbumDAO
    ...
    Public Function ReadAll() As Object
        ...
        For Each aux In col
            ar = New Album(CInt(aux(1).ToString))
            ar.SetAlbumName(aux(2).ToString)
            ar.SetAlbumYear(CInt(aux(3).ToString))
            ar.SetAlbumArtist(CInt(aux(4).ToString))
            Me.Albums.Add(ar)
        Next
        Return Albums
    End Function
    ...
    Public Function Insert(ByVal ar As Album) As Integer
        Return DBBroker.GetBroker.Change("INSERT INTO Albums (AlbumName, YearAlbum, Artist) VALUES ('" & ar.GetAlbumName() & "' , (SELECT idArtist FROM artists WHERE idArtist='" & ar.GetAlbumArtist() & "'), '" & ar.GetAlbumArtist & "');")

    End Function

    Public Function Update(ByVal ar As Album) As Integer
        Return DBBroker.GetBroker.Change("UPDATE Albums SET albumName='" & ar.GetAlbumName() & "', albumYear='" & ar.GetAlbumYear() & "', albumArtist=" & ar.GetAlbumArtist & " " & "WHERE idAlbum=" & ar.GetIdAlbum() & ";")
