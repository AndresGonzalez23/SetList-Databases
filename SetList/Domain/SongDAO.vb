Public Class SongDAO
    ...
    Public Function Query2(path As String) As Object
        ...
        Dim col As Collection = DBBroker.GetBroker().Read("SELECT DISTINCT s.sName, count(s.sName) FROM songs s, playbacks p WHERE (p.song=s.IdSong) GROUP BY s.sName ORDER BY count(s.sName);")
        For Each aux In col
            name = aux(1).ToString
            names.Add(name)
        Next
    ...
    Public Function Query4(path As String) As Object
        Dim names As New Collection : Dim aux As Collection
        Dim col As Collection = DBBroker.GetBroker().Read("SELECT us.uName , SUM(s.length) FROM users us, playbacks p, songs s WHERE(us.Email=p.user and p.song = s.IdSong) GROUP BY us.uName ORDER BY SUM(s.length) desc;")
        Dim name As String
        For Each aux In col
            name = aux(1).ToString
            names.Add(name)
